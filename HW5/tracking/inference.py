...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()
        if noisyDistance == None: # if ghost is captured
            prison = self.getJailPosition()
            allPossible[prison] = 1.0
        else:
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for p in self.legalPositions: # ghost's position
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: # if ghost is captured
            prison = self.getJailPosition()
            self.particles = [prison] * self.numParticles
        else:
            allPossible = util.Counter()
            ...
            if allPossible.totalCount() == 0: # all particles receive 0 weight
                self.initializeUniformly(gameState) # changes self.particles
            else:
                newParticles = []
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        beliefs = util.Counter() # key, value = position, probability
        for p in self.particles:
            beliefs[p] += 1.
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        shuffled_positions = list(itertools.product(self.legalPositions, repeat = self.numGhosts))


        

        #Borrowed from previous problems!
        particles = []
        #Maybe legal = len(shuffled_positions)
        legal = len(shuffled_positions)
        random.shuffle(shuffled_positions)
        for p in shuffled_positions:
            for i in range(self.numParticles / legal):
                particles.append(p)
        ...
        for p in self.particles:
            ...
            for index in range(self.numGhosts):
                ...
                if noisyDistances[index] == None:
                    ...
                    distance = util.manhattanDistance(p[index], pacmanPosition)
                    change = change * emissionModels[index][distance]
            allPossible[p] = allPossible[p] + change

        for ind, i in enumerate(allPossible.values()):

            if i:
                allPossible.normalize()
                self.particles = []
                for ind2 in range(self.numParticles):
                    self.particles.append(util.sample(allPossible))
                break
            elif len(allPossible.values())-1 == ind:
                self.initializeParticles()
                for p in self.particles:
                    for index in range(self.numGhosts):
                        if noisyDistances[index] == None:
                            i = self.getParticleWithGhostInJail(p, index)
